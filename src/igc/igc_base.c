 ...
 */
static int32_t igc_reset_hw_base(struct igc_hw *hw)
{
	int32_t ret_val;
	uint32_t ctrl;
 ...
 * @hw: pointer to the HW structure
 */
static int32_t igc_init_nvm_params_base(struct igc_hw *hw)
{
 ...
 */
static int32_t igc_setup_copper_link_base(struct igc_hw *hw)
{
	int32_t  ret_val = 0;
	uint32_t ctrl;
 ...
 * @hw: pointer to the HW structure
 */
static int32_t igc_init_mac_params_base(struct igc_hw *hw)
{
 ...
 * @hw: pointer to the HW structure
 */
static int32_t igc_init_phy_params_base(struct igc_hw *hw)
{
...
static int32_t igc_get_invariants_base(struct igc_hw *hw)
{
	int32_t ret_val = 0;
 ...
 * function pointer entry point called by the api module.
 */
static int32_t igc_acquire_phy_base(struct igc_hw *hw)
{
 ...
 */
static int32_t igc_init_hw_base(struct igc_hw *hw)
{
	struct igc_mac_info *mac = &hw->mac;
...
{
	uint32_t rctl, rlpml, rxdctl, rfctl, temp_rctl, rx_enabled;
	int ms_wait;
	...
	/* Disable Rx queue */
	rxdctl = rd32(IGC_RXDCTL);
	wr32(IGC_RXDCTL, rxdctl & ~IGC_RXDCTL_QUEUE_ENABLE);
	
	/* Poll all queues to verify they have shut down */
	for (ms_wait = 0; ms_wait < 10; ms_wait++) {
		usleep(2000);
		rx_enabled = 0;
		rx_enabled |= rd32(IGC_RXDCTL);
	...
	/* Re enable Rx queue if it were previously enabled and restore our
	 * previous state
	 */
